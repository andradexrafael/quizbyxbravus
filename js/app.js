...
const answerFinalStrings = {
  '1': {
    ...
    desc: 'ta carente na quarentena sem o crush, mas ta tudo bem, em breve tudo isso passa e volta crush, volta amigos!!!',

  },
  '2': {
    img: 'img/naoequalquerum.png',
    result: '“Não é qualquer um que bate na minha raba”',
    desc: 'personalidade forte, imprevisível e enxerga a realidade de uma forma diferenciada. Oh yeah'
  },
  '3': {
    img: 'img/quedaquiapouco.png',
    result: '“Que daqui a pouco eu tenho que ralar, mas vou ralar pensando no que a gente fez”',
...
function showQuestion(id) {
  Array.from(document
    .querySelectorAll(`[class^=quiz-container]:not(.${currentQuestion})`))
    .forEach(element => {
...
function changeAnswer(event) {
    const {dataset: {value, answer}} = event.target;
...
window.onload = () => {
    Array.from(document
        ...
        .forEach(element => element.addEventListener('click', changeAnswer));

    localStorage.setItem('@GamaQuiz:quiz-who-you-are', JSON.stringify(answerFinalStrings));
}
